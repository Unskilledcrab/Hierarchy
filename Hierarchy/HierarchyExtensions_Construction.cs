...
namespace Hierarchy
{
    ...
    public static partial class HierarchyExtensions
    {
        ...
        private static IEnumerable<THierarchyModel> ToHierarchyInternal<THierarchyModel, TData, TKey>(this IEnumerable<TData> flatList, Func<TData, TKey> idSelector, Func<TData, TKey> parentIdSelector, TKey rootId = default)
            where THierarchyModel : IHierarchyNode<TData>, new()
        {
            ...
            foreach (var item in lookup.Values)
            {
                ...
                {
                    throw new ArgumentException($"The item with id {parentId} has matching parentId and Id. A hierarchy list can not contain cyclic items. If you are wanting to use cyclic data, use the {nameof(GraphExtensions.ToGraph)} Extension method", nameof(flatList));
                }
        ...
        public static List<THierarchyModel> ToHierarchy<THierarchyModel, TData, TKey>(this IEnumerable<TData> flatList, Func<TData, TKey> idSelector, Func<TData, TKey> parentIdSelector, TKey rootId = default)
            where THierarchyModel : IHierarchyNode<TData>, new()
        {
            ...
            return flatList.ToHierarchyInternal<THierarchyModel, TData, TKey>(idSelector, parentIdSelector, rootId).ToList();
        }

        public static List<HierarchyNode<TData>> ToHierarchy<TData, TKey>(this IEnumerable<TData> flatList, Func<TData, TKey> idSelector, Func<TData, TKey> parentIdSelector, TKey rootId = default)
        {
            return flatList.ToHierarchy<HierarchyNode<TData>, TData, TKey>(idSelector, parentIdSelector, rootId);
        }

        public static List<HierarchyNode<TData>> ToHierarchy<TData, TKey>(this IEnumerable<TData> flatList, TKey rootId = default)
            where TData : IHierarchy<TKey>
        {
            return flatList.ToHierarchy<HierarchyNode<TData>, TData, TKey>(f => f.Id, f => f.ParentId, rootId);
        }
    }
}
